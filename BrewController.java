		...
		if(model.isCreative()) {
			if(model.getStage() == BrewModel.STAGE_BASE) {
				...
				for(int i = 0; i < model.baseCount(); i++) {
					view.addChoice(model.baseLabel(i, currentPlayer), model.ownsBase(i, currentPlayer));
				}
			} else {
				view.setHeading("Throw in 1 to 3 fruits, then hit BREW");
				for(int i = 0; i < model.fruitCount(); i++) {
					view.addChoice(model.fruitLabel(i, currentPlayer), model.canAddFruit(i, currentPlayer, cauldron));
				}
			}
		} else {
			...
			for(int i = 0; i < unlocked.size(); i++) {
				view.addChoice(model.recipeLabel(unlocked.get(i)), currentPlayer.hasSufficientIngredients(unlocked.get(i)));
			}
		}
