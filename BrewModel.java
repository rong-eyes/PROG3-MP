...
public class BrewModel { 
	...
	*/
	public String recipeLabel(Recipe r) {
		//the base is left out because the name of the concoction already says which one it uses
		return String.format("[%03d] %s  -  %d crystals  (%s)", r.getConcoctionID(), r.getName(),
				r.getPrice(), Spellbook.ingredientsToString(r));
	}
	...
	*/
	...
	/**
	* Returns the drawing of the concoction that was just brewed.
	* <p>
	* Only a successful brew has something to show, so a failed experiment and every rejected brew return
	* null. A concoction that has not been drawn yet returns null as well, and the result is then reported
	* with words alone.
	* </p>
	*
	* @param status the result returned by the player's brewing
	* @param p the player who did the brewing
	* @return the file location of the concoction's drawing; null if there is nothing to show
	*/
	public String brewImage(int status, Player p) {
		if(status != Player.BREW_SUCCESS)
			return null;

		return ItemArt.potionPathOf(p.getLastConcoction());
	}
