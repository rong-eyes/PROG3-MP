...
public class MarketModel { //Model for the Market screens; wraps the Market so the screens never touch it directly
	...
	private int brewsOnArrival;		//the brew count as it stood before this visit reset it
	...
	*/
	public boolean visitMarket() {
		this.brewsOnArrival = market.getBrewsSinceVisit();

		return market.refreshIfNeeded();
	}

	/**
	* Writes the line that tells the player how the restock is coming along.
	* <p>
	* Arriving at the market always resets the brew count, so the number reported here is the one the player
	* walked in with. It tells them either that their brewing earned them new stock, or how many more
	* concoctions they still need to brew before coming back.
	* </p>
	*
	* @return the restock line to be displayed on the market screen
	*/
	public String restockMessage() {
		if(brewsOnArrival >= REFRESH_THRESHOLD)
			return "You brewed " + brewsOnArrival + " concoctions since your last visit,<br>"
					+ "so the merchant laid out new wares.";

		int stillNeeded = REFRESH_THRESHOLD - brewsOnArrival;

		return "Brews since your last visit: " + brewsOnArrival + " of " + REFRESH_THRESHOLD + "<br>"
				+ "Brew " + stillNeeded + " more before returning for new wares.";
	}
	...
	*/
	...
	public int getBrewsOnArrival() {
		return brewsOnArrival;
	}
