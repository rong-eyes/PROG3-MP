...
public class MarketPanel extends JPanel { //the View for the market, where the 8 slots are displayed
	...
	public MarketPanel() {
		...
		crystals.setBounds(145, 48, 158, 26);
		crystals.setFont(new Font("Times New Roman", Font.BOLD, 25));
		crystals.setForeground(SIGN_INK);
		this.add(crystals);

		//HOW THE RESTOCK IS COMING ALONG, WRITTEN ACROSS THE MIDDLE OF THE TABLE CLOTH
		restockNote = new JLabel("", SwingConstants.CENTER);
		restockNote.setBounds(160, 670, 520, 50);
		restockNote.setVerticalAlignment(SwingConstants.TOP);
		restockNote.setFont(new Font("Times New Roman", Font.BOLD, 14));
		restockNote.setForeground(SHELF_INK);
		this.add(restockNote);
	...
	*/
	...
	/**
	* Displays how many concoctions were brewed before this visit and whether that earned a restock.
	*
	* @param text the restock line to be displayed
	*/
	public void setRestockNote(String text) {
		restockNote.setText("<html><div style='text-align:center; width:510px;'>" + text + "</div></html>");
	}
